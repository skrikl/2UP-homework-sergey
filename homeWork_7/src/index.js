...
function skipDefault(eventName, target) {
    target.addEventListener(eventName, target => target.preventDefault());
}
...
function emulateClick(target) {
  target.dispatchEvent(event = new Event('click'));
}
...
function once(target, fn) {
  target.addEventListener('click', function removeUserEvent() {
    fn();
    target.removeEventListener('click', removeUserEvent);
  });
}
